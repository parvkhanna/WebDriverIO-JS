...
class AbflLoan extends BasePage {
    ...
    get ddSchoolFeeBand() {
        return $("//label[@for='School Fee Band']/../div/div")
    }
    get ddStudentGrade() {
        return $("//label[@for='Student Grade']/../div/div")
    }
    get tfStudentsAddress() {
    ...
    get btnCheckCibilScore() {
        return $("//button[text()='Check Cibil Score']")
    }
    get btnAbflHistory() {
        return $("//a[@href='/abfl']/..//button[text()='History']")
    }
    get btnProceed() {
    ...
    btnResume(appId) {
        return $(`//a[@href='/abfl?appId=${appId}']`)
    }

    get btnAbflHistoryUnprocessed() {
        return $("//a[@href='/abfl']/..//div/a[text()='Unprocessed']")
    }
    getAppId(panNumber) {
    ...
    get btnNextTab() { 
        return $("(//button[text()='Next'])[1]") 
    }
    get btnSubmit() {
        return $("//button[(text()='Submit')]")
    }

    btnDocumentVarificationLibtnprocessedAbflenk(appId) {
        return $("//a[@href='/manage-loan/unprocessed/abfl/approveLoan/docapproved/" + appId + "']")
    }
    get btnPreview() {
    ...
    getddResidenceTypeOptions(index) { return $(`//div[@class='css-11unzgr']/div[contains(@id,"option-${index}")]`) }

    getddstudentGradeOptions(index) { return $(`//div[@class='css-11unzgr']/div[contains(@id,"option-${index}")]`) }
    ...
    get btnUnprocessedAbfl() {
         return $("//a[@href='/manage-loan/unprocessed/abfl']") 
    }

    get btnprocessedAbfl() {
         return $("//a[@href='/manage-loan/processed/abfl']") 
    }
    ...
    get btnDocReceivedAndFinOpsApprove() { 
            return $("//button[text()='Doc Received & Fin Ops Approve']") 
        }
    get btnclose() {
    ...
    getddCustomerOccupationOptions(index) {
        return $(`//div[@class='css-11unzgr']/div[contains(@id,"option-${index}")]`)
    }
    ...
    getddQualificationOptions(index) {
        return $(`//div[@class='css-11unzgr']/div[contains(@id,"option-${index}")]`)
    }
    ...
    async loginPaymentPage(username, password) {
        ...
        await this.btnAbfl.waitForExist({ timeout: 60000 });
    }
    ...
    async customerDetails(data,appId) {
        //await this.btnAbfl.click();
        await this.btnSkipUploadDocument.waitForClickable({ timeout: 60000 })
        ...
        //await this.tfPanNumber.setValue(data.panNumber);
        ...
        //await this.tfPhoneNumber.setValue(data.telephoneNumber);
        await this.loadingPage.waitForExist({ timeout: 20000, reverse: true });
        await this.tfEmailAddress.setValue(data.email);
        await this.tfAddressField.waitForEnabled({ timeout: 20000 });
        await this.tfAddressField.setValue(data.studentsAddress);
        await this.tfStudentsPinCode.setValue(data.studentPinCode);
        await this.tfLoanAmount.waitForClickable({ timeout: 55000 });
    ...
    async enterCustomerAndLoanDetails(data) {
        let appId = await this.createTlpAppId(abflData.abflValidData);
        await this.customerDetails(data,appId)
        //Additional details 
        await this.ddOccupationType.waitForClickable();
    ...
    async clickOnProceedButton() {
        ...
        try { await this.btnProceed.waitForDisplayed({ timeout: 60000 }) }
        catch { }
    ...
    async returnAppId(panNumber) {
        await this.btnAbflHistory.click();
        await this.btnAbflHistoryUnprocessed.click();
    ...
    async verifyOKYC(data) {
        ...
        await browser.debug()
        await this.rbNonAadharKyc.waitForExist({ timeout: 40000 });
    ...
    async uploadDocuments(data) {
        ...
        await this.tfPanNumber.setValue(data);
        await this.btnContinue.click();
        let nonAadharKycVisibilityStatus = await this.rbNonAadharKyc.isDisplayed();
        if (nonAadharKycVisibilityStatus) {
            await this.verifyOKYC(data);
        }
    ...
    async lmsVerifyDocuments(appId) {
        ...
        await this.btnUnprocessedAbfl.click();
        await this.btnAbbTicketSearchBox.setValue(appId);
        await browser.keys("Enter");
...
async lmsVerifyDocumentsForFinOpsApproved(appId) 
{
    ...
    await this.btnManageLoan.waitForExist({ timeout: 30000 });
    await this.btnManageLoan.click();
    await this.btnprocessedAbfl.waitForDisplayed();
    await this.btnprocessedAbfl.click();
    await this.btnAbbTicketSearchBox.setValue(appId);
    await browser.keys("Enter");
    await mongoConnect.abflAddOrderId(appId)
    await browser.refresh()
